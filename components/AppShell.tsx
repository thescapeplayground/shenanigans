...
import { NavigationContext, ActiveTab } from "@/components/NavigationContext";

const navItems: { id: ActiveTab; label: string; href: string; icon: typeof Home }[] = [
...
export function useActiveTab(pathname: string): ActiveTab {
  if (pathname === "/") return "home";
  return pathname.replace(/^\//, "") as ActiveTab;
}
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // Tab state used only on the home page for client-side tab switching
  const [homeTab, setHomeTab] = useState<ActiveTab>("home");

  // When the user navigates directly to /, reset homeTab to "home"
  useEffect(() => {
    if (isHome) setHomeTab("home");
  }, [isHome]);
  ...
  return (
    <NavigationContext.Provider value={{ activeTab, homeTab, isHome, onTabChange }}>
              ...
              >
                ...
                >
                  {navItems.map((item) => {
                    ...
                    const isActive = isHome ? homeTab === item.id : activeTab === item.id;
                    const shared = (
                      <>
                        {isActive && (
                          <motion.div
                    ...
                    return (
                      <div
                        ...
                        {...(isHome
                          ? { onClick: () => onTabChange(item.id), role: "button", tabIndex: 0 }
                          : {})}
                      >
                        {isHome ? (
                          shared
                        ) : (
                          <Link href={item.href}>{shared}</Link>
                        )}
                      </div>
                    );
                  })}
